...
# CNFG_DIR = os.path.dirname(__file__)
CNFG_DIR = os.path.join(
    os.environ.get('ERRBOT_CFG_DIR'), 'plugins', 'err-sponsors'
)
DATA_DIR = os.path.join(
    os.environ.get('ERRBOT_BASE_DIR'), 'data', 'err-sponsors'
)

CONFIG_TEMPLATE = {
    'GOOGLE_API_KEYFILE': os.path.join(CNFG_DIR, 'google_api_key.json'),
    ...
    'CONTRACTS_DIR': os.path.join(DATA_DIR, 'agreements'),
    'TEMPLATE_FILE': {
        'eps': os.path.join(CNFG_DIR, 'sponsor_agreement_template_eps.tex'),
        'aps': os.path.join(CNFG_DIR, 'sponsor_agreement_template_aps.tex'),
    }
}
...
class SponsorsPlugin(BotPlugin):
    ...
    def configure(self, configuration):
        if configuration:
            config = dict(
                chain(CONFIG_TEMPLATE.items(), configuration.items())
            )
        else:
    ...
    def pick_one_sponsor(sponsors, sponsor_name):
        try:
            ...
            raise KeyError(
                'Could not find data for sponsor {}.'.format(sponsor_name)
            )
        else:
            if len(sponsor_data) != 1:
                raise KeyError(
                    "Found more than one sponsor: {}.".format(
                        sponsor_data.to_json())
                )
            return sponsor_data
    ...
    def sponsor_info(self, msg, company):
        ...
        return tabulate(
            [(col, sponsor_data[col].values[0]) for col in info_cols],
            tablefmt='pipe'
        )
    ...
    def _get_contract_template(self, contract_type):
        ...
        raise KeyError(
            'Could not find a contract for contract_type {}.'.format(
                contract_type)
        )
